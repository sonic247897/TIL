...
8. 읽은 내용을 사용자 프로그램에게 직접 전달하는 것이 아니라, `운영체제`가 먼저 자신의 커널 메모리 영역에 일부를 저장한다.
   ...
   >   
   >   - 이미 `메모리`에 올라와 있는 페이지에 대해서는 **운영체제**가 중간에 끼어들지 못하고, 주소 변환을 HW가 전부 하여 `메모리`에 바로 접근한다. 
   >   
   >     `page fault`가 발생할 때만 CPU가 **운영체제**로 넘어가서 페이지를 `swap area`에서 `메모리`로 읽어온다.
   >   
   >     (요청한 페이지가 존재하는지 여부를 **MMU**가 판단한다)
   >   
...
- Access Control 방법
  ...
  1. Access control Matrix
     ...
     >
     > => 빈 칸은 list에 연결하지 않아도 되지만, 이 방법도 부가적인 **오버헤드**가 너무 크다.
...
- Unified Buffer Cache
  ...
  - **최근의 OS(ex. LINUX)에서는 기존의 buffer cache가 page cache에 통합됨**
    ...
    > **`운영체제`에서 물리적 메모리(page frame)를 관리하는 루틴에 page cache와 buffer cache를 같이 관리한다는 뜻**으로, 통합되었다고 해서 각 캐시를 관리할 수 있는 방법이 바뀌는 것은 아니다.
  ...
  *예전 OS(ex. FreeBSD)의 물리적 메모리와 물리적 디스크*
  ...
  - 버퍼 캐시는 <u>커널 메모리 영역</u>에서 존재하며, 파일을 **블록 단위**로 관리한다.
    ...
    => LINUX와 같은 최근의 OS에서 사용하는 `Unified Buffer Cache`에서는 블록 크기도 4 KB로 관리한다.
...
기존의 버퍼 캐시 환경에서는 두 가지 `인터페이스` 모두 **버퍼 캐시**를 거쳤었다.

따라서 `Memory-mapped I/O 방식`의 경우 페이지 캐시에 한번 복사해야 하는 오버헤드가 있었다.

Unified Buffer Cache에서는 운영체제가 따로 버퍼 캐시 공간을 만들어 놓지 않고, 필요에 따라 페이지 캐시를 할당해서 공간을 사용한다.

=> `Memory-mapped I/O 방식`에서 페이지 캐시 자체가 사용자 프로세스의 논리적 주소 영역 페이지에 매핑되어서, 버퍼 캐시를 별도로 거치지 않고 페이지 캐시에 직접 읽고 쓸 수 있다.
